---
title: Wholesome Egg Masala Noodles
translationKey: egg-masala-noodles
slug: egg-masala-noodles
summary: A homestyle egg noodles recipe with cabbage, pepper, tomato, and warm masala, made without artificial taste boosters.
category: non-veg
tags:
  - noodles
  - egg
  - dinner
  - kids
  - homemade
featured: true
published: 2026-07-12
updated: 2026-07-12
difficulty: Easy
servings: 4
prepTime: 15 min
cookTime: 15 min
totalTime: 30 min
cover: BIRYANI
ingredients:
  - group: Egg scramble
    name: Eggs
    quantity: 3
  - group: Egg scramble
    name: Oil
    quantity: 1
    unit: tbsp
  - group: Noodles base
    name: Oil
    quantity: 4
    unit: tbsp
  - group: Noodles base
    name: Big onion
    quantity: 1
    note: Sliced lengthwise
  - group: Noodles base
    name: Grated cabbage
    quantity: 200
    unit: g
  - group: Spices
    name: Turmeric powder
    quantity: 0.5
    unit: tsp
  - group: Spices
    name: Black pepper powder
    quantity: 1.5
    unit: tsp
  - group: Spices
    name: Salt
    note: As required
  - group: Spices
    name: Garam masala
    quantity: 1.5
    unit: tsp
    note: Or use noodles masala
  - group: Finish
    name: Boiled noodles
    quantity: 300
    unit: g
  - group: Finish
    name: Big tomato
    quantity: 1
    note: Ground into paste
  - group: Optional
    name: Soy sauce
    quantity: 3
    unit: tbsp
    note: Use instead of tomato for a stronger street-style taste
nutrition:
  protein: From eggs
  vegetables: Onion and cabbage
---

This is the first recipe I am sharing from my own kitchen notes, so I wanted it to feel close to home: simple, filling, and still mindful about what goes into the wok. These egg masala noodles are made with everyday ingredients, plenty of cabbage, fresh tomato paste, pepper, and homemade-style masala.

The taste is warm and familiar, but the method is fast enough for a busy evening. The most important part is the high-flame stir-fry. Once the cabbage goes in, stay near the wok and keep stirring. That little attention gives the noodles their lively texture.

{% recipeImage "BIRYANI_STEP1", "Temporary cover-style image", "Biryani image is used as a temporary placeholder. Replace it later with the egg noodles image." %}

## Before You Start

Boil the noodles first, drain them, and keep them ready. Do not overcook them. Soft but firm noodles mix better in the wok and will not become sticky.

## Method

1. Heat 1 tablespoon oil in a wok. Add 3 eggs and scramble them into small pieces. Once cooked, transfer the scrambled eggs to a plate and keep them aside.
2. In the same wok, add 4 tablespoons oil. Add the sliced onion and fry until it starts changing color lightly.
3. Add the grated cabbage and stir-fry on high flame for about 7 minutes. Do not leave the wok unattended; keep stirring continuously so the cabbage cooks evenly without burning.
4. Add turmeric powder, black pepper powder, salt, and garam masala. If garam masala is not available, use noodles masala. Stir for 2 minutes, then reduce the flame.
5. Add the boiled noodles. Increase the flame again and toss everything together for 1 minute.
6. Add the tomato paste and stir for another minute. If you do not have tomato, or if you want a stronger taste, add 3 tablespoons soy sauce instead.
7. Add the scrambled eggs back into the wok, mix gently, and switch off the stove.

## Jothi's Kitchen Note

I prepare my own garam masala at home. That secret masala deserves its own post, so I will share it later. For now, use a good garam masala if you have it, or use a small amount of noodles masala.

## Serve

Serve these hot noodles with love, especially for children after school or as a quick family dinner. A simple cucumber salad or a bowl of curd on the side will keep the meal balanced.
